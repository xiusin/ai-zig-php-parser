...
const reflection = main.runtime.reflection;
...
test "Attribute creation and basic functionality" {
    ...
    try testing.expect(attribute.arguments[0].data.integer == 42);
    try testing.expect(std.mem.eql(u8, attribute.arguments[1].data.string.data.data, "test"));
...
test "ReflectionAttribute functionality" {
    ...
    try testing.expect(attr_args[0].data.integer == 42);
    try testing.expect(std.mem.eql(u8, attr_args[1].data.string.data.data, "test"));
