...
pub const NativeLinker = struct {
    ...
    /// 生成全局变量
    fn generateGlobalVariable(self: *Self, writer: anytype, global: *const IR.Global) !void {
        // 生成全局变量声明
    ...
    fn irTypeToZigTypeString(self: *const Self, ir_type: IR.Type) []const u8 {
        ...
        return switch (ir_type) {
            ...
            .ptr => "runtime.Value",  // 指针类型也返回Value
            else => "runtime.Value",
        };
    }
                ...
                if (block_idx + 1 < func.blocks.items.len) {
                    ...
                    if (cleanup_regs.len > 0) {
                        ...
                        for (cleanup_regs) |reg_id| {
                            const cleanup = try std.fmt.allocPrint(self.allocator,
                                "                reg_{d}.release(runtime.runtime_allocator);\n",
                                .{reg_id});
    ...
    fn generateTerminatorSimple(self: *Self, code: *std.ArrayList(u8), term: IR.Terminator, cleanup_regs: []const usize, func: *const IR.Function, _: usize, func_has_return_value: bool) !void {
        switch (term) {
            .ret => |ret_val| {
                ...
                if (ret_val) |reg| {
                    const ret_stmt = try std.fmt.allocPrint(self.allocator,
                        "                return reg_{d};\n",
                        .{reg.id});
            ...
            .cond_br => |br| {
                ...
                const cond = try std.fmt.allocPrint(self.allocator,
                    "                if (reg_{d}) {{\n                    current_block = {d};\n                }} else {{\n                    current_block = {d};\n                }}\n",
    ...
    fn generateInstructionDirect(self: *Self, code: *std.ArrayList(u8), inst: *const IR.Instruction) !void {
        ...
        switch (inst.op) {
            ...
            .const_string => |string_id| {
                const instr = try std.fmt.allocPrint(self.allocator,
                    "        reg_{d} = runtime.Value.initString(try runtime.PHPString.init(runtime.runtime_allocator, string_table[{d}]));\n",
            ...
            .store => |op| {
                ...
                if (ptr_tag == .php_value) {
                    const release = try std.fmt.allocPrint(self.allocator,
                        "        reg_{d}.*.release(runtime.runtime_allocator);\n",
                        .{op.ptr.id});
            ...
            .call => |op| {
                ...
                // 其他指令暂时跳过
                const comment = try std.fmt.allocPrint(self.allocator,
                    "        // TODO: {s}\n",
    ...
    fn copyRuntimeLib(self: *Self, temp_dir: []const u8) !void {
        ...
        if (self.config.verbose) {
            std.debug.print("  Copied runtime library: {s} -> {s}\n", .{ template_path, runtime_path });
        }
    }
