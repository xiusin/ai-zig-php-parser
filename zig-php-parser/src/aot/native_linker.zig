...
pub const NativeLinker = struct {
    ...
    fn generateClassRegistration(self: *Self, writer: anytype, ir_module: *const IR.Module) !void {
        ...
        for (0..class_count) |ci| {
            ...
            for (0..method_counts[ci]) |mi| {
                ...
                const full_name = try std.fmt.bufPrint(&full_name_buf, "{s}::{s}", .{ cname, mname });
    ...
    fn generateFunction(self: *Self, code: *std.ArrayList(u8), _: *const IR.Module, func: *const IR.Function) !void {
        std.debug.print("=== generateFunction: {s}, blocks={d} ===\n", .{func.name, func.blocks.items.len});
        ...
        if (func.blocks.items.len == 1) {
            // 单基本块：直接生成线性代码
            std.debug.print("Single block path: {d} instructions\n", .{func.blocks.items[0].instructions.items.len});
            ...
            std.debug.print("Multi-block path: {d} blocks\n", .{func.blocks.items.len});
    ...
    fn writeRegAssignmentFmt(
        ...
        // 调试：检测所有 initInt
        if (std.mem.indexOf(u8, format_str, "initInt") != null) {
            std.debug.print("REG_{d}: {s} | args: {any}\n", .{reg_id, format_str, args});
        }
    ...
    fn generateInstructionSimple(self: *Self, code: *std.ArrayList(u8), inst: *const IR.Instruction) !void {
        ...
        // 调试：跟踪所有指令
        if (inst.result) |reg| {
            std.debug.print("INST: {s} -> reg_{d}\n", .{@tagName(inst.op), reg.id});
        } else {
            std.debug.print("INST: {s} (no result)\n", .{@tagName(inst.op)});
        }
        ...
        switch (inst.op) {
            ...
            .shl => |op| {
                if (inst.result) |reg| {
                    try self.writeRegAssignmentFmt(writer, reg.id, "reg_{d} << @as(u6, @intCast(reg_{d}));\n", .{ op.lhs.id, op.rhs.id });
                }
            },
            .shr => |op| {
                if (inst.result) |reg| {
            ...
            .select => |op| {
                if (inst.result) |reg| {
                    const type_tag = @as(std.meta.Tag(IR.Type), reg.type_);
                    if (type_tag == .i64) {
                        try self.writeRegAssignmentFmt(writer, reg.id, "if (reg_{d}) reg_{d} else reg_{d};\n", .{ op.cond.id, op.then_value.id, op.else_value.id });
                    } else if (type_tag == .f64) {
                        try self.writeRegAssignmentFmt(writer, reg.id, "if (reg_{d}) reg_{d} else reg_{d};\n", .{ op.cond.id, op.then_value.id, op.else_value.id });
                    } else if (type_tag == .bool) {
                        try self.writeRegAssignmentFmt(writer, reg.id, "if (reg_{d}) reg_{d} else reg_{d};\n", .{ op.cond.id, op.then_value.id, op.else_value.id });
            ...
            .property_set => |op| {
                ...
                const escaped_prop = try self.escapeString(op.property_name);
                defer self.allocator.free(escaped_prop);
                try writer.print("    try runtime.php_object_set(reg_{d}, \"{s}\", ", .{ op.object.id, escaped_prop });
                try self.writePhpValueExpr(writer, value_type_tag, op.value.id);
                try writer.writeAll(");\n");
            },
            .method_call => |op| blk: {
            ...
            .static_property_set => |op| {
                ...
                const escaped_prop = try self.escapeString(op.property_name);
                defer self.allocator.free(escaped_prop);
                try writer.print("    try runtime.php_set_static_property(\"{s}\", \"{s}\", ", .{ escaped_class, escaped_prop });
                try self.writePhpValueExpr(writer, value_type_tag, op.value.id);
                try writer.writeAll(");\n");
            },
            // ============ 异常处理指令 ============
            ...
            .cast => |op| {
                // cast: 类型转换
                if (inst.result) |reg| {
                    std.debug.print("CAST: reg_{d} = cast reg_{d} from {s} to {s}\n", .{
                        reg.id,
                        op.value.id,
