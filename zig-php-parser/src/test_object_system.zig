...
test "PHPClass creation and basic functionality" {
    ...
    // Create a class
    var php_class = try types.PHPClass.init(allocator, class_name);
    defer php_class.deinit(allocator);
...
test "PHPClass property management" {
    ...
    try testing.expect(retrieved_prop.?.default_value.?.getTag() == .integer);
    try testing.expect(retrieved_prop.?.default_value.?.asInt() == 42);
}
...
test "PHPObject creation and property access" {
    ...
    // Create an object
    var php_object = try types.PHPObject.init(allocator, &php_class);
    defer php_object.deinit(allocator);

    // Test property access
    const prop_value = try php_object.getProperty("testProperty");
    try testing.expect(prop_value.getTag() == .integer);
    try testing.expect(prop_value.asInt() == 100);

    // Test property setting
    try php_object.setProperty(allocator, "testProperty", types.Value.initInt(200));
    const updated_value = try php_object.getProperty("testProperty");
    try testing.expect(updated_value.asInt() == 200);

    // Test dynamic property
    try php_object.setProperty(allocator, "dynamicProperty", types.Value.initInt(300));
    const dynamic_value = try php_object.getProperty("dynamicProperty");
    try testing.expect(dynamic_value.asInt() == 300);
}
...
test "PHPObject method checking" {
    ...
    // Test method checking through class
    try testing.expect(php_object.class.hasMethod("testMethod"));
    try testing.expect(!php_object.class.hasMethod("nonExistentMethod"));
}
...
test "VM class registration and object creation" {
    ...
    try testing.expect(object_value.getTag() == .object);
    try testing.expect(std.mem.eql(u8, object_value.getAsObject().data.class.name.data, "TestClass"));

    // Clean up the object manually for this test
    object_value.getAsObject().data.deinit(allocator);
    allocator.destroy(object_value.getAsObject().data);
    allocator.destroy(object_value.getAsObject());
}
...
test "Object cloning" {
    ...
    var original_object = try types.PHPObject.init(allocator, &php_class);
    defer original_object.deinit(allocator);

    try original_object.setProperty(allocator, "testProp", types.Value.initInt(42));
    ...
    try testing.expect(cloned_prop.asInt() == 42);

    // Test that modifying clone doesn't affect original
    try cloned_object.setProperty(allocator, "testProp", types.Value.initInt(100));
    const original_prop = try original_object.getProperty("testProp");
    try testing.expect(original_prop.asInt() == 42);
}
