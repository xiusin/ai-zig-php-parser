...
test "Object system integration with VM" {
    ...
    try testing.expect(person_object.tag == .object);

    // Test property access
    const name_value = try vm.getObjectProperty(person_object, "name");
    try testing.expect(name_value.tag == .string);
    try testing.expect(std.mem.eql(u8, name_value.data.string.data.data, "Unknown"));

    const age_value = try vm.getObjectProperty(person_object, "age");
    try testing.expect(age_value.tag == .integer);
    try testing.expect(age_value.data.integer == 0);
    ...
    try testing.expect(std.mem.eql(u8, updated_name.data.string.data.data, "John Doe"));

    // Test method checking
    try testing.expect(person_object.data.object.data.hasMethod("__construct"));
    try testing.expect(!person_object.data.object.data.hasMethod("nonExistentMethod"));

    // Clean up
    person_object.data.object.data.deinit(allocator);
    allocator.destroy(person_object.data.object.data);
    allocator.destroy(person_object.data.object);
}
...
test "Class inheritance functionality" {
    ...
    // Create objects
    var parent_object = types.PHPObject.init(allocator, &parent_class);
    defer parent_object.deinit();

    var child_object = types.PHPObject.init(allocator, &child_class);
    defer child_object.deinit();

    // Test object inheritance
    try testing.expect(child_object.isInstanceOf(&parent_class));
    try testing.expect(child_object.isInstanceOf(&child_class));
    try testing.expect(!parent_object.isInstanceOf(&child_class));

    // Test property access on child object
    const species_value = try child_object.getProperty("species");
    try testing.expect(species_value.tag == .string);
    try testing.expect(std.mem.eql(u8, species_value.data.string.data.data, "Unknown"));

    const breed_value = try child_object.getProperty("breed");
    try testing.expect(breed_value.tag == .string);
    try testing.expect(std.mem.eql(u8, breed_value.data.string.data.data, "Mixed"));
}
