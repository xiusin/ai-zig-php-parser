...
const CoreContext = @import("core/common.zig").CoreContext;

/// 时间内置函数实现
/// Time builtin functions implementation
pub const TimeBuiltins = struct {
    /// PHP time() - Returns current Unix timestamp（调用 core_time）
    /// Requirements: 1.1
    ...
    /// PHP microtime() - Returns current time with microseconds（调用 core_time）
    /// Requirements: 1.4, 1.5
    pub fn microtime(vm: *anyopaque, args: []const Value) !Value {
        const vm_ptr = @as(*@import("vm.zig").VM, @ptrCast(@alignCast(vm)));
        const get_as_float = if (args.len >= 1) blk: {
            break :blk if (args[0].getTag() == .boolean)
                args[0].asBool()
        ...
        if (get_as_float) {
            ...
            var ctx = CoreContext{ .allocator = vm_ptr.allocator };
            const formatted = try core_time.microtime_string(&ctx);
