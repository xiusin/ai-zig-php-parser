...
pub const NumberWrapper = struct {
    ...
    pub fn toString(self: NumberWrapper, allocator: std.mem.Allocator) !*types.PHPString {
        return switch (self.value) {
            .integer => |v| types.PHPString.init(allocator, std.fmt.allocPrint(allocator, "{d}", .{v}) catch ""),
            .float => |v| types.PHPString.init(allocator, std.fmt.allocPrint(allocator, "{d}", .{v}) catch ""),
        };
    }
};
