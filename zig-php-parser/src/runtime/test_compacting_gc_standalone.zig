...
const PropertyTest = struct {
    ...
    fn run(
        ...
        while (i < self.iterations) : (i += 1) {
            ...
            const input = try generator(&rng, self.allocator);
...
/// 生成随机内存块序列
fn generateMemoryBlocks(rng: *std.Random, allocator: std.mem.Allocator) !MemoryBlockSequence {
...
/// 生成随机空闲块序列（可能相邻）
fn generateFreeBlocks(rng: *std.Random, allocator: std.mem.Allocator) !FreeBlockSequence {
...
fn coalesceFreeBlocks(blocks: []const FreeBlockSequence.FreeBlock, allocator: std.mem.Allocator) ![]FreeBlockSequence.FreeBlock {
    ...
    // 合并相邻块
    var merged = try std.ArrayList(FreeBlockSequence.FreeBlock).initCapacity(allocator, blocks.len);
