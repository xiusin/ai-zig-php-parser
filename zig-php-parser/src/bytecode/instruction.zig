...
pub const OpCode = enum(u8) {
    ...
    // 参数传递优化指令 (0x97-0x9F)
    pass_by_value = 0x97, // 值传递 - 复制参数
    pass_by_ref = 0x98, // 引用传递 - 传递指针
    pass_by_cow = 0x99, // COW传递 - 共享直到修改
    pass_by_move = 0x9A, // 移动传递 - 转移所有权
    cow_check = 0x9B, // COW检查 - 检查是否需要复制
    cow_copy = 0x9C, // COW复制 - 执行写时复制
    ret_move = 0x9D, // 移动返回 - RVO优化
    ret_cow = 0x9E, // COW返回 - 返回共享值
    ...
    pub fn isTerminator(self: OpCode) bool {
        return switch (self) {
            .ret, .ret_void, .ret_move, .ret_cow, .throw, .rethrow, .halt => true,
            else => self.isJump(),
        };
    }
};
