...
- **已修复**：寄存器收集问题 ✅
  - 现在收集所有操作数寄存器（BinaryOp, UnaryOp, Terminator）
  - `reg_1`, `reg_2`, `reg_4` 等寄存器现在正确声明
- **剩余问题**：类型推断错误
  - IR 生成器将某些 Value 类型推断为 f64/i64
  - 例如：`php_add` 返回 Value，但被推断为 f64
  - 这是 IR 生成器的类型推断问题，不是代码生成器问题
- **影响**：不影响简单脚本和大多数实际应用
- **解决方案**：需要完善 IR 生成器的类型推断逻辑
