...
echo "✓ file_get_contents: 读取成功\n";

// file_exists
echo "✓ file_exists: " . (file_exists($test_file) ? "true" : "false") . "\n";
echo "✓ file_exists (不存在的文件): " . (file_exists("/tmp/nonexistent_file.txt") ? "true" : "false") . "\n";

// is_file
echo "✓ is_file: " . (is_file($test_file) ? "true" : "false") . "\n";
...
echo "✓ basename (带路径): " . basename("/tmp/test.txt") . "\n";

// dirname
echo "✓ dirname: " . dirname($test_file) . "\n";
echo "✓ dirname (深层): " . dirname("/tmp/a/b/c.txt") . "\n";
...
echo "✓ scandir: 找到 " . count($files) . " 个条目\n";
...
echo "✓ file_get_contents (不存在的文件): 返回 " . ($nonexistent === false ? "false" : "其他值") . "\n";

// 删除不存在的文件
unlink("/tmp/nonexistent_file_12345.txt");
echo "✓ unlink (不存在的文件): 已处理\n";

// 打开不存在的文件
$fp = fopen("/tmp/nonexistent_file_12345.txt", "r");
echo "✓ fopen (不存在的文件): 返回 " . ($fp === false ? "false" : "其他值") . "\n";
