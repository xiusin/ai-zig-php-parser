...
pub const SIMDArray = struct {
    ...
    pub fn arrayMapInt(self: *const SIMDArray, arr: []const i64, map_fn: *const fn (i64) i64) ![]i64 {
    ...
    /// @post 返回映射后的新数组
    /// @performance 2-3x 标量实现（对于简单映射函数）
    pub fn arrayMapFloat(self: *const SIMDArray, arr: []const f64, map_fn: *const fn (f64) f64) ![]f64 {
    ...
    pub fn arrayFilterInt(self: *const SIMDArray, arr: []const i64, filter_fn: *const fn (i64) bool) ![]i64 {
        if (arr.len == 0) return &[_]i64{};
    ...
    /// @post 返回过滤后的新数组
    /// @performance 2-3x 标量实现（对于简单过滤函数）
    pub fn arrayFilterFloat(self: *const SIMDArray, arr: []const f64, filter_fn: *const fn (f64) bool) ![]f64 {
        if (arr.len == 0) return &[_]f64{};
