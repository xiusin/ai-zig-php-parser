...
pub const CrashHandler = struct {
    ...
    fn handleCrash(sig: c_int, info: *const std.posix.siginfo_t, ucontext: ?*anyopaque) void {
        ...
        const filename = std.fmt.bufPrint(&filename_buf, "crash_{d}_{d}.txt", .{ context.process_id, context.timestamp }) catch "crash_report.txt";
        ...
        if (global_handler) |handler| {
            ...
            const path = std.fmt.bufPrint(&path_buf, "{s}/{s}", .{ handler.crash_report_dir, filename }) catch return;
