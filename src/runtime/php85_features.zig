...
// 注意：registerUriFunctions() 已移除 — 所有函数通过 fn_dispatch comptime 数据注册
// 新增函数只需在 fn_table.zig 添加声明 + 在 fn_dispatch.zig 的 HANDLER_MAP_KVS 添加 handler

pub fn uriParseFn(vm: *VM, args: []const Value) !Value {
...
pub fn uriBuildFn(vm: *VM, args: []const Value) !Value {
...
pub fn uriResolveFn(vm: *VM, args: []const Value) !Value {
