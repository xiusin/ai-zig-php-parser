...
pub fn scandirComplete(vm: *VM, args: []const Value) !Value {
    if (args.len < 1) {
        const exception = try ExceptionFactory.createArgumentCountError(vm.allocator, "scandir() expects at least 1 parameter", "builtin", 0);
    ...
    try entries.append(try DirEntry.init(vm.allocator, ".", dot_stat.kind, dot_stat.size, dot_stat.mtime));

    // 添加 ".." 条目
    const dotdot_stat = dir.statFile("..") catch {
        return Value.initBool(false);
    };
    try entries.append(try DirEntry.init(vm.allocator, "..", dotdot_stat.kind, dotdot_stat.size, dotdot_stat.mtime));
    ...
    while (try iter.next()) |entry| {
        ...
        try entries.append(try DirEntry.init(vm.allocator, entry.name, entry.kind, stat.size, stat.mtime));
    }
...
pub fn globFn(vm: *VM, args: []const Value) !Value {
    if (args.len < 1) {
        const exception = try ExceptionFactory.createArgumentCountError(vm.allocator, "glob() expects at least 1 parameter", "builtin", 0);
        _ = try vm.throwException(exception);
        return error.InvalidArgumentCount;
    }
...
pub fn pathinfoFn(vm: *VM, args: []const Value) !Value {
    if (args.len < 1) {
        const exception = try ExceptionFactory.createArgumentCountError(vm.allocator, "pathinfo() expects at least 1 parameter", "builtin", 0);
        _ = try vm.throwException(exception);
        return error.InvalidArgumentCount;
    }
...
pub fn tempnamFn(vm: *VM, args: []const Value) !Value {
    if (args.len < 2) {
        const exception = try ExceptionFactory.createArgumentCountError(vm.allocator, "tempnam() expects 2 parameters", "builtin", 0);
        _ = try vm.throwException(exception);
        return error.InvalidArgumentCount;
    }
    ...
    if (dir.getTag() != .string or prefix.getTag() != .string) {
        const exception = try ExceptionFactory.createTypeError(vm.allocator, "tempnam() expects parameters 1 and 2 to be string", "builtin", 0);
        _ = try vm.throwException(exception);
        return error.InvalidArgumentType;
    }
    ...
    while (attempts < 100) : (attempts += 1) {
        ...
        const filename = try std.fmt.bufPrint(&filename_buf, "{s}/{s}{x}", .{ dir_str, prefix_str, random_num });
...
pub fn tmpfileFn(vm: *VM, args: []const Value) !Value {
    ...
    var filename_buf: [std.fs.MAX_PATH_BYTES]u8 = undefined;
    const filename = try std.fmt.bufPrint(&filename_buf, "/tmp/php_tmp_{x}", .{random_num});
